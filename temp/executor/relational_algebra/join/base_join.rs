...
impl<S: StorageClient> BaseJoinExecutor<S> {
        ...
        if !self.is_hash_table_built {
            ...
            for row in &dataset.rows {
                ...
                for hash_key in hash_key_exprs {
                    let key =
                        ExpressionEvaluator::evaluate(hash_key, &mut row_context).map_err(|e| {
    ...
    pub fn get_cached_single_key_hash_table(&self) -> Option<&HashMap<Value, Vec<Vec<Value>>>> {
        self.cached_single_key_hash_table.as_ref()
    }

    /// Retrieve the cached multi-key hash table (must be built first).
    pub fn get_cached_multi_key_hash_table(&self) -> Option<&HashMap<JoinKey, Vec<Vec<Value>>>> {
