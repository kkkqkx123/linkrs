...
use crate::query::executor::base::{ApplyConfig, BaseExecutor, ExecutionResult, ExecutorConfig};
...
impl<S: StorageClient + Send + 'static> ApplyExecutor<S> {
    ...
    pub fn with_context(
        ...
        Self {
            base: BaseExecutor::with_context(id, "ApplyExecutor".to_string(), storage, context),
            left_input_var: config.left_input_var,
            right_input_var: config.right_input_var,
            correlated_cols: config.correlated_cols,
            col_names: config.col_names,
    ...
    fn build_correlated_key<C: ExpressionContext>(&self, expr_context: &mut C) -> DBResult<List> {
    ...
    fn collect_valid_keys(
        &self,
        right_values: &[Value],
        expr_context: &mut DefaultExpressionContext,
    ) -> DBResult<HashMap<usize, List>> {
    ...
    fn collect_valid_single_keys(
        &self,
        right_values: &[Value],
        expr_context: &mut DefaultExpressionContext,
    ) -> DBResult<HashMap<usize, Value>> {
        let mut valid_keys = HashMap::new();
