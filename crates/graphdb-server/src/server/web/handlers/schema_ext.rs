...
>(
    ...
    let properties: Vec<graphdb_api::api::core::PropertyDef> = request
        .get("properties")
        ...
        .map(|arr| {
            arr.iter()
                .filter_map(|p| {
                    ...
                    Some(graphdb_api::api::core::PropertyDef {
                        name: name.to_string(),
                        data_type,
    ...
    let result = task::spawn_blocking(move || {
        ...
        let additions = if let Some(props) = request.add_properties {
            ...
            for p in props {
                ...
                core_props.push(graphdb_api::api::core::PropertyDef {
                    name: p.name,
                    data_type,
                    nullable: p.nullable,
                    default_value: None,
...
>(
    ...
    let result = task::spawn_blocking(move || {
        ...
        let target = match request.entity_type.as_str() {
            "TAG" => graphdb_api::api::core::IndexTarget::Tag {
                name: request.entity_name,
                fields: request.fields,
            },
            "EDGE" => graphdb_api::api::core::IndexTarget::Edge {
                name: request.entity_name,
                fields: request.fields,
            },
            _ => {
