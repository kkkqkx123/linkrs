...
impl<S: StorageClient + Clone + 'static> BatchManager<S> {
    ...
    async fn process_items(
        ...
        let core_items: Vec<graphdb_api::api::core::BatchItem> = items
            .into_iter()
            ...
            errors: core_result
                ...
                .map(|e| BatchErrorData {
                    ...
                    item_type: match e.item_type {
                        graphdb_api::api::core::BatchItemType::Vertex => BatchItemType::Vertex,
                        graphdb_api::api::core::BatchItemType::Edge => BatchItemType::Edge,
                    },
                    error: e.message,
                })
                .collect(),
    ...
    fn convert_to_core_item(&self, item: BatchItem) -> Option<graphdb_api::api::core::BatchItem> {
        match item {
            BatchItem::Vertex(data) => self
                .convert_vertex_data(data)
                .map(graphdb_api::api::core::BatchItem::Vertex),
            BatchItem::Edge(data) => self
                .convert_edge_data(data)
                .map(graphdb_api::api::core::BatchItem::Edge),
        }
    }
