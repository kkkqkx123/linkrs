...
pub(super) fn scan_vertices(
    n: &LogicalScanVerticesNode,
...
pub(super) fn scan_edges(n: &LogicalScanEdgesNode) -> Result<FactorizedSchema, FactorizationError> {
    let mut schema = FactorizedSchema::new();
    let g = schema.create_flat_group(false);
    if let Some(expr) = &n.expression {
        let eid = resolve_id(expr);
...
pub(super) fn get_edges(n: &LogicalGetEdgesNode) -> Result<FactorizedSchema, FactorizationError> {
    let mut schema = FactorizedSchema::new();
    let g = schema.create_flat_group(false);
...
pub(super) fn start() -> Result<FactorizedSchema, FactorizationError> {
    let mut schema = FactorizedSchema::new();
    schema.create_flat_group(false);
    Ok(schema)
}
