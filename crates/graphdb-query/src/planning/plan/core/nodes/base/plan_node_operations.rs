...
macro_rules! match_all_nodes_with_default {
    ($self:expr, $method:ident, $default:expr) => {
        match $self {
            ...
            PlanNodeEnum::CorrelatedApply(node) => node.$method(),
...
impl PlanNodeEnum {
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ...
            PlanNodeEnum::Flatten(_) => "Flatten",
            ...
            PlanNodeEnum::CorrelatedApply(_) => "CorrelatedApply",
    ...
    pub fn output_var(&self) -> Option<&str> {
        match self {
            ...
            PlanNodeEnum::CorrelatedApply(node) => node.output_var(),
    ...
    pub fn dependencies_ref(&self) -> Cow<'_, [&PlanNodeEnum]> {
        match self {
            ...
            PlanNodeEnum::Window(node) => Cow::Owned(vec![node.input()]),
            PlanNodeEnum::DataCollect(node) => Cow::Owned(vec![node.input()]),
            PlanNodeEnum::Dedup(node) => Cow::Owned(vec![node.input()]),
            PlanNodeEnum::Flatten(node) => Cow::Owned(vec![node.input()]),
            PlanNodeEnum::PatternApply(node) => Cow::Owned(vec![node.input()]),
            PlanNodeEnum::CorrelatedApply(node) => Cow::Owned(vec![node.input()]),
    ...
    pub fn set_output_var(&mut self, var: String) {
        match self {
            ...
            PlanNodeEnum::CorrelatedApply(node) => node.set_output_var(var),
