...
use crate::planning::plan::explain::description::PlanNodeDescription;
...
use crate::planning::plan::core::nodes::graph_operations::set_operations_node::{
    IntersectNode, MinusNode,
};
use crate::planning::plan::core::nodes::graph_operations::window_node::WindowNode;
...
impl PlanNodeVisitor for DescribeVisitor {
    ...
    // Filter / Assign / Aggregate / Window are generated by hand below so
    // they can surface the expression-level subquery count (`subquery: N`)
    // and the constant-folding marker (`folded: true`).

    fn visit_filter(&mut self, node: &FilterNode) {
        let deps = self.visit_children_single(node);
        let mut desc = PlanNodeDescription::new("Filter", node.id());
        desc.set_dependencies(deps);
        let count = node.subqueries().len();
        if count > 0 {
            desc.add_description("subquery", count.to_string());
        }
        if node.has_folded_expressions() {
            desc.add_description("folded", "true");
        }
    ...
    fn visit_window(&mut self, node: &WindowNode) {
        let deps = self.visit_children_single(node);
        let mut desc = PlanNodeDescription::new("Window", node.id());
        if let Some(var) = node.output_var() {
            desc = desc.with_output_var(var.to_string());
        }
        desc.set_dependencies(deps);
