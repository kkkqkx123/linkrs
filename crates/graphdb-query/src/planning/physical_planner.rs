...
// ==================== Per-operator lowering helpers ====================
//
// Each logical operator lowers in its own function so the recursive
// converter above keeps a small stack frame: only one operator's
// temporaries are live per plan level instead of every operator's.
...
fn convert_multi_shortest_path(
...
fn convert_pipe_delete_vertices(
