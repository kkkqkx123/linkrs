...
impl WithClausePlanner {
    ...
    fn apply_order_by(
        ...
        let sort_items: Vec<crate::planning::plan::core::nodes::SortItem> = order_by_ctx
            ...
            .map(|(idx, dir)| {
                ...
                crate::planning::plan::core::nodes::SortItem::column(column, *dir)
            })
            .collect();
    ...
    fn extract_with_context(stmt: &Stmt) -> Result<WithClauseContext, PlannerError> {
        ...
        Ok(WithClauseContext {
            ...
            where_clause: with_stmt.where_clause.clone().map(|condition| {
                crate::binder::validation::WhereClauseContext {
