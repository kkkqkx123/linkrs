        ...
        match node {
            ...
            PlanNodeEnum::SemiMasker(node) => {
                ...
                let mask_keys = node
                    .mask_keys()
                    .iter()
                    .map(|s| crate::core::Value::String(s.clone().into()))
                    .collect();
                let spec = crate::query::executor::streaming::operators::spec::FactorizedSpec::SemiMasker {
                    key_slot: 0,
                    mask_keys,
                    keep_match: node.keep_match(),
                };
                Self::push_factorized_op(
                    operators,
                    fragments,
                    op_alloc,
                    frag_alloc,
            ...
            PlanNodeEnum::MultiplicityReducer(node) => {
                ...
                let group_key_slots = (0..node.group_key_columns().len()).collect();
                let spec = crate::query::executor::streaming::operators::spec::FactorizedSpec::MultiplicityReducer {
                    group_key_slots,
                };
                Self::push_factorized_op(
                    operators,
            ...
            PlanNodeEnum::NodeLabelFilter(node) => {
                ...
                let spec = crate::query::executor::streaming::operators::spec::FactorizedSpec::NodeLabelFilter {
                    label_slot: 0,
                    allowed_labels: node.allowed_labels().to_vec(),
                };
                Self::push_factorized_op(
                    operators,
                    fragments,
