    ...
    match executor {
        StreamingExecutor::GroupBy {
            ...
            if result_iter.is_none() {
                ...
                // Create result rows - one per group (just take first row from each group)
                let result_rows: Vec<Vec<Value>> =
                    groups.into_values().map(|mut g| g.pop().unwrap()).collect();
    ...
    match executor {
        StreamingExecutor::WindowFunction {
            ...
            if result_iter.is_none() {
                ...
                // For now, just pass through rows
                // Window function computation would go here
                let result_rows = all_rows.clone();
                *result_iter = Some(result_rows.into_iter());
            }
