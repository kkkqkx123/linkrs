...
use crate::query::executor::result_processing::transformations::{
    AppendVerticesExecutor, ApplyExecutor, AssignExecutor, PatternApplyExecutor,
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let (variant_name, exec_name) = match self {
            ...
            ExecutorEnum::GraphOperation(exec) => match exec {
                ...
                GraphOperationExecutor::ShortestPath(e) => {
                    ("GraphOperation::ShortestPath", e.name())
                }
                GraphOperationExecutor::MultiShortestPath(e) => {
                    ("GraphOperation::MultiShortestPath", e.name())
                }
            ...
            ExecutorEnum::ResultProcessing(exec) => match exec {
                ...
                ResultProcessingExecutor::Materialize(e) => {
                    ("ResultProcessing::Materialize", e.name())
                }
                ResultProcessingExecutor::AppendVertices(e) => {
                    ("ResultProcessing::AppendVertices", e.name())
                }
                ResultProcessingExecutor::RollUpApply(e) => {
                    ("ResultProcessing::RollUpApply", e.name())
                }
                ResultProcessingExecutor::PatternApply(e) => {
                    ("ResultProcessing::PatternApply", e.name())
                }
...
use macros::{
    call_input_executor_get_input, call_input_executor_set_input, delegate_to_executor,
    delegate_to_executor_mut,
};
