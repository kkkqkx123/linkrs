...
impl<S: StorageClient + Send + 'static> Debug for ExecutorEnum<S> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let (variant_name, exec_name) = match self {
            ...
            ExecutorEnum::Window(exec) => ("Window", exec.name()),
            ...
            ExecutorEnum::Apply(exec) => ("Apply", exec.name()),
...
impl<S: StorageClient + Send + 'static> InputExecutor<S> for ExecutorEnum<S> {
    fn set_input(&mut self, input: ExecutorEnum<S>) {
        match self {
            ...
            ExecutorEnum::Window(exec) => exec.set_input(input),
    ...
    fn get_input(&self) -> Option<&ExecutorEnum<S>> {
        match self {
            ...
            ExecutorEnum::Window(exec) => exec.get_input(),
...
impl<S: StorageClient + Send + 'static> NodeType for ExecutorEnum<S> {
    ...
    fn category(&self) -> NodeCategory {
        match self {
            ...
            ExecutorEnum::Window(_) => NodeCategory::Aggregate,
            ...
            ExecutorEnum::Apply(_) => NodeCategory::Other,
    ...
    macro_rules! delegate_to_executor {
        ($self:expr, $method:ident) => {
            match $self {
                ...
                ExecutorEnum::Window(exec) => exec.$method(),
                ...
                ExecutorEnum::Apply(exec) => exec.$method(),
