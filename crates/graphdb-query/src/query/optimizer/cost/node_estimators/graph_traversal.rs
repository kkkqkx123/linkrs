...
impl<'a> GraphTraversalEstimator<'a> {
    ...
    fn get_avg_out_degree(&self, space: &str, edge_type: Option<&str>) -> f64 {
        edge_type
            .and_then(|et| {
                self.cost_calculator
                    .statistics_manager()
    ...
    fn get_avg_in_degree(&self, space: &str, edge_type: Option<&str>) -> f64 {
        edge_type
            .and_then(|et| {
                self.cost_calculator
                    .statistics_manager()
    ...
    fn get_avg_degree(&self, space: &str, edge_type: Option<&str>) -> f64 {
        edge_type
            .and_then(|et| {
                self.cost_calculator
                    .statistics_manager()
    ...
    fn get_edge_stats(&self, space: &str, edge_type: Option<&str>) -> Option<EdgeTypeStatistics> {
    ...
    fn calculate_skew_aware_expand_cost(
        ...
        let stats = self.get_edge_stats(space, edge_type);
        ...
        match stats {
            Some(s) if s.is_heavily_skewed() => {
                ...
                let base_cost = self
                    .cost_calculator
                    .calculate_expand_cost(space, start_rows, edge_type);
