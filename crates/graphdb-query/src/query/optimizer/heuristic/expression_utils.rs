...
fn rewrite_expression_with_map(
    ...
    match expr {
        ...
        Expression::Aggregate {
            ...
            func: func.clone(),
            args: args.iter().map(|a| rewrite_expression_with_map(a, rewrite_map, expr_context.clone())).collect(),
            distinct: *distinct,
            filter: filter
                .as_ref()
                .map(|f| Box::new(rewrite_expression_with_map(f, rewrite_map, expr_context))),
        },
