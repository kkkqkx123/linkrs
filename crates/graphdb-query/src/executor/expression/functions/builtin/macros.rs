    ...
    ($name:ident, $op:expr, $desc:literal) => {
        fn $name(
            ...
            match &args[0] {
                ...
                _ => Err($crate::executor::expression::ExpressionError::type_error(
                    concat!($desc, "Functions require numeric types"),
                )),
            }
        }
    };
    ...
    ($name:ident, $op:expr, $desc:literal) => {
        fn $name(
            ...
            match &args[0] {
                ...
                Value::Null(_) => Ok(Value::Null(NullType::Null)),
                _ => Err($crate::executor::expression::ExpressionError::type_error(
                    concat!($desc, "Functions require a string type"),
                )),
            }
        }
    };
    ...
    ($name:ident, $op:expr, $desc:literal) => {
        fn $name(
            ...
            match (&args[0], &args[1]) {
                ...
                (Value::Null(_), _) | (_, Value::Null(_)) => Ok(Value::Null(NullType::Null)),
                _ => Err($crate::executor::expression::ExpressionError::type_error(
                    concat!($desc, "Functions require geo-typed parameters"),
                )),
            }
        }
    };
