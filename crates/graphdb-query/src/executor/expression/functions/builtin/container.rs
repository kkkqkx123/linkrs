...
fn execute_size(args: &[Value]) -> Result<Value, ExpressionError> {
    ...
    match &args[0] {
        ...
        Value::JsonB(j) => match j.as_value() {
            serde_json::Value::Object(m) => Ok(Value::BigInt(m.len() as i64)),
            serde_json::Value::Array(a) => Ok(Value::BigInt(a.len() as i64)),
            _ => Ok(Value::BigInt(1)),
        },
...
fn execute_range(args: &[Value]) -> Result<Value, ExpressionError> {
    ...
    if step == 0 {
        return Err(ExpressionError::new(
            crate::executor::expression::ExpressionErrorType::InvalidOperation,
...
fn execute_list_append(args: &[Value]) -> Result<Value, ExpressionError> {
    ...
    match &args[0] {
        Value::List(list) => {
            ...
            Ok(Value::list(List { values: new_values }))
        }
        Value::Null(_) => Ok(Value::Null(NullType::Null)),
        _ => Err(ExpressionError::type_error(
    ...
    match (&args[0], &args[1]) {
        (Value::List(list), Value::List(mask)) => {
            ...
            let filtered: Vec<Value> = list.values[..min_len]
                ...
                .map(|(v, _)| v.clone())
                .collect();
            Ok(Value::list(List { values: filtered }))
        }
