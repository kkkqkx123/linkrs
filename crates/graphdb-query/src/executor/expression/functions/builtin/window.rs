...
fn execute_lag(args: &[Value]) -> Result<Value, ExpressionError> {
    ...
    let offset = match &args[1] {
        Value::Int(i) => *i as i64,
        Value::BigInt(i) => *i,
        _ => return Err(ExpressionError::type_error("lag offset must be an integer")),
    };
    if offset < 0 {
        return Err(ExpressionError::type_error(
...
fn execute_nth_value(args: &[Value]) -> Result<Value, ExpressionError> {
    ...
    if n < 1 {
        return Err(ExpressionError::type_error("nth_value n must be positive"));
    }
    Ok(args[0].clone())
}
...
fn execute_ntile(args: &[Value]) -> Result<Value, ExpressionError> {
    ...
    let n = match &args[0] {
        Value::Int(i) => *i as i64,
        Value::BigInt(i) => *i,
        _ => return Err(ExpressionError::type_error("ntile n must be an integer")),
    };
    if n < 1 {
        return Err(ExpressionError::type_error("ntile n must be positive"));
    }
    Ok(Value::BigInt(1))
}
