...
pub fn aggregate_from_str_with_args(
    ...
    match func_name.to_uppercase().as_str() {
        ...
        "PERCENTILE_CONT" => {
            if args.len() < 2 {
                return Err(ExpressionError::function_error(
                    "PERCENTILE_CONT function requires a field name and percentile value"
                        .to_string(),
                ));
            }
...
impl AggregateExpression {
    ...
    pub fn evaluate<C: crate::executor::expression::ExpressionContext>(
        &self,
        context: &mut C,
        state: &mut AggregateState,
    ) -> Result<Value, ExpressionError> {
        let arg_value =
            crate::executor::expression::evaluator::expression_evaluator::ExpressionEvaluator::evaluate(
                &self.argument,
                context,
            )
