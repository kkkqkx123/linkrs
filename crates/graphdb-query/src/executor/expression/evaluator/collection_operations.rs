...
fn json_subscript_access(json: &JsonValue, index: &Value) -> Result<Value, ExpressionError> {
    match (json, index) {
        ...
        (JsonValue::Object(_), _) => Err(ExpressionError::type_error(
            "JSON object key must be a string",
        )),
        (JsonValue::Array(_), _) => Err(ExpressionError::type_error(
            "JSON array index must be an integer",
        )),
        _ => Err(ExpressionError::type_error(
            "Subscript not supported on this JSON type",
        )),
    }
}
        ...
        JsonValue::Array(arr) => {
            if let Ok(index) = property.parse::<isize>() {
                ...
                Err(ExpressionError::type_error(
                    "JSON array property must be an integer index",
                ))
            }
        }
...
impl CollectionOperationEvaluator {
    ...
    pub fn eval_subscript_access(
        ...
        match collection {
            ...
            Value::JsonB(j) => json_subscript_access(j.as_value(), index),
            _ => Err(ExpressionError::type_error(
    ...
    pub fn eval_property_access(object: &Value, property: &str) -> Result<Value, ExpressionError> {
        ...
        match object {
            ...
            Value::JsonB(j) => json_property_access(j.as_value(), property),
            _ => Err(ExpressionError::type_error(
