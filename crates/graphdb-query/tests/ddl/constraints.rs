...
fn test_default_value_execution_insert() {
    TestScenario::new()
        ...
        .exec_ddl("CREATE TAG Person(id: INT, name: STRING, age: INT DEFAULT 18)")
        .assert_success()
...
fn test_default_value_string_execution() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Person(id: INT, name: STRING DEFAULT 'unknown')")
        .assert_success()
...
fn test_default_with_not_null_constraint() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Person(id: INT, name: STRING NOT NULL DEFAULT 'unknown', age: INT)")
        .assert_success()
        .exec_dml("INSERT VERTEX Person(age) VALUES 1:(30)")
        .assert_success()
        .assert_vertex_props(
