...
// ==================== GO Execution Tests ====================
//
// Single-label note: GO syntax carries no vertex label. Seeds and neighbors
// are resolved at execution from the edge type schema, so fixtures declare
// endpoint labels on the edge types.
...
fn test_go_execution_reversely() {
    TestScenario::new()
        ...
        .assert_success()
        .query("GO FROM 1 OVER FOLLOWS REVERSELY")
        .assert_success()
        .assert_result_count(2);
}
...
fn test_go_execution_bidirect() {
    TestScenario::new()
        ...
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Person(id INT, name STRING)")
        .exec_ddl("CREATE EDGE FRIEND(since DATE) FROM Person TO Person")
        .exec_dml("INSERT VERTEX Person(name) VALUES 1:('Alice'), 2:('Bob'), 3:('Charlie')")
        .exec_dml("INSERT EDGE FRIEND(since) VALUES 1 -> 2:('2020-01-01'), 3 -> 1:('2021-01-01')")
        .assert_success()
        .query("GO FROM 1 OVER FRIEND BIDIRECT")
        .assert_success()
        .assert_result_count(2);
}
...
fn test_go_execution_basic_traversal() {
    TestScenario::new()
        ...
        .assert_success()
        .assert_result_count(2)
        .query("GO 2 STEPS FROM 1 OVER KNOWS YIELD $$.Person.name AS friend_of_friend")
        .assert_success();
}
...
fn test_go_nonexistent_edge() {
    // An unknown edge type cannot determine the seed domain, so the query
