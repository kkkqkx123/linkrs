...
fn test_group_by_with_aggregation() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Product(id INT, name STRING, category STRING, price DOUBLE)")
        .assert_success()
        .exec_dml(
            r#"
...
#[test]
fn test_return_list_comprehension() {
    TestScenario::new()
        .expect("Failed to create test scenario")
...
#[test]
fn test_return_list_comprehension_in_vertex_projection() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_comprehension")
        .exec_ddl("CREATE TAG person(id INT, name STRING)")
        .assert_success()
        .exec_dml("INSERT VERTEX person(name) VALUES 1:('Alice')")
        .assert_success()
...
fn test_return_distinct() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Person(id INT, name STRING, city STRING)")
        .assert_success()
        .exec_dml(
            r#"
...
fn test_pipe_with_fetch() {
    TestScenario::new()
        ...
        .exec_dml("INSERT EDGE KNOWS(since) VALUES 1 -> 2:('2020-01-01')")
        .assert_success()
        .query("GO FROM 1 OVER KNOWS | YIELD target.name AS name")
        .assert_success()
        .assert_result_count(1)
