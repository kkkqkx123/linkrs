...
impl Fixture {
    ...
    fn create_space_with_person_tag(&mut self, space_name: &str) -> graphdb_query::core::types::SpaceInfo {
        self.pipeline
...
fn test_memo_hit_skips_bind_on_warm_statement() {
    ...
    fx.insert(&space, "INSERT VERTEX person(name, age) VALUES \"p1\": (\"A\", 1)");
    assert_eq!(
    ...
    fx.insert(&space, "INSERT VERTEX person(name, age) VALUES \"p2\": (\"B\", 2)");
    assert_eq!(
        fx.pipeline.dml_bind_skipped_count(),
        1,
    ...
    assert_eq!(fx.name_of("memo_space", "p1"), Some(graphdb_query::core::Value::string("A")));
    assert_eq!(fx.name_of("memo_space", "p2"), Some(graphdb_query::core::Value::string("B")));
}
...
fn test_ddl_invalidation_restores_bind_path() {
    ...
    fx.insert(&space, "INSERT VERTEX person(name, age) VALUES \"p3\": (\"C\", 3)");
    assert_eq!(
        fx.pipeline.dml_bind_skipped_count(),
        1,
    ...
    fx.insert(&space, "INSERT VERTEX person(name, age) VALUES \"p4\": (\"D\", 4)");
    assert_eq!(
        fx.pipeline.dml_bind_skipped_count(),
        2,
    ...
    assert_eq!(fx.name_of("ddl_space", "p3"), Some(graphdb_query::core::Value::string("C")));
    assert_eq!(fx.name_of("ddl_space", "p4"), Some(graphdb_query::core::Value::string("D")));
}
...
fn test_space_switch_restores_bind_path() {
    ...
    fx.insert(&space_a, "INSERT VERTEX person(name, age) VALUES \"p1\": (\"A\", 1)");
    fx.insert(&space_a, "INSERT VERTEX person(name, age) VALUES \"p2\": (\"B\", 2)");
    assert_eq!(fx.pipeline.dml_bind_skipped_count(), 1);

    fx.insert(&space_b, "INSERT VERTEX person(name, age) VALUES \"q1\": (\"X\", 9)");
    assert_eq!(
        fx.pipeline.dml_bind_skipped_count(),
        1,
    ...
    fx.insert(&space_b, "INSERT VERTEX person(name, age) VALUES \"q2\": (\"Y\", 8)");
    assert_eq!(
        fx.pipeline.dml_bind_skipped_count(),
        2,
    ...
    assert_eq!(fx.name_of("space_a", "p1"), Some(graphdb_query::core::Value::string("A")));
    assert_eq!(fx.name_of("space_a", "p2"), Some(graphdb_query::core::Value::string("B")));
    assert_eq!(fx.name_of("space_b", "q1"), Some(graphdb_query::core::Value::string("X")));
    assert_eq!(fx.name_of("space_b", "q2"), Some(graphdb_query::core::Value::string("Y")));
    assert!(fx.storage.read().get_vertex("space_b", &VertexId::from_string("p1")).expect("read").is_none());
}
...
fn test_param_type_change_restores_bind_path() {
    ...
    fx.insert(&space, "INSERT VERTEX person(name, age) VALUES \"p3\": (\"C\", 3.0)");
    assert_eq!(
        fx.pipeline.dml_bind_skipped_count(),
        1,
    ...
    assert_eq!(fx.name_of("param_type_space", "p3"), Some(graphdb_query::core::Value::string("C")));
    let p3 = fx
        .storage
        .read()
        ...
        .expect("read")
        .expect("exists");
    assert_eq!(p3.properties.get("age"), Some(&graphdb_query::core::Value::Double(3.0)));
}
