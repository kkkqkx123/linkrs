...
fn test_insert_not_null_success() {
    TestScenario::new()
        ...
        .exec_ddl("CREATE TAG Person(id INT, name STRING NOT NULL, age INT)")
        .assert_success()
...
fn test_insert_multiple_not_null_columns() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Person(id INT, name STRING NOT NULL, email STRING NOT NULL, age INT)")
        .assert_success()
...
fn test_insert_default_uses_default_value() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Person(id INT, name STRING DEFAULT 'Anonymous', age INT DEFAULT 18)")
        .assert_success()
...
fn test_insert_default_all_defaults() {
    TestScenario::new()
        .expect("Failed to create test scenario")
        .setup_space("test_space")
        .exec_ddl("CREATE TAG Person(id INT, name STRING DEFAULT 'Anonymous', age INT DEFAULT 30)")
        .assert_success()
