...
impl VertexTable {
    ...
    /// Strict delta apply used when a commit manifest pins the checkpoint.
    /// Unlike the legacy path, every manifest-listed page must decode:
    /// a bad name, an unknown column, or a corrupt payload refuses the
    /// open instead of running with silently skipped data.
    pub fn apply_delta_pages_strict(&mut self, shard_dir: &Path) -> StorageResult<()> {
        let delta_dir = shard_dir.join("columns_pages");
        if !delta_dir.exists() {
            return Ok(());
        }
