...
use super::format::{parse_header, snapshot_error, write_snapshot_file};
use super::MappedFrozen;
use crate::edge::ImmutableCsr;

impl MappedFrozen {
    /// Open and validate a snapshot file. Any structural problem is an error;
    ...
    /// Open and validate a snapshot file under a declared memory intent.
    ///
    /// `ReadSnapshot` explicitly requests transparent huge pages on Linux for
    ...
    pub fn open_with_intent(
        ...
        let file = File::open(path)
            .map_err(|e| StorageError::io_error(format!("snapshot file open failed: {e}")))?;
        let map = unsafe { memmap2::Mmap::map(&file) }
            .map_err(|e| StorageError::io_error(format!("snapshot file map failed: {e}")))?;
        // Transparent huge pages only on the declared read-snapshot path.
        // The hint is best-effort: rejection falls back to base pages
        // without failing the open.
        #[cfg(target_os = "linux")]
        if intent == MemoryIntent::ReadSnapshot {
    ...
    /// Open the snapshot file, rebuilding it from `frozen` when it is missing
    /// or fails validation.
    pub fn open_or_rebuild(path: &Path, frozen: &ImmutableCsr) -> StorageResult<Self> {
        match Self::open(path) {
            Ok(mapped) => Ok(mapped),
            Err(_) => {
                write_snapshot_file(frozen, path)?;
                Self::open(path)
            }
        }
    }
    ...
    fn from_map(map: Arc<memmap2::Mmap>) -> StorageResult<Self> {
        ...
        let degree_bytes = map
            ...
            .ok_or_else(|| {
                snapshot_error(format!(
                    "snapshot degree column out of range at byte {}",
                    columns.degrees.start
                ))
            })?;
        if degree_bytes.len() != rows.saturating_mul(4) {
            return Err(snapshot_error(format!(
                "snapshot degree column length mismatch: holds {} bytes, layout needs {}",
                degree_bytes.len(),
        ...
        for chunk in degree_bytes.as_chunks::<4>().0 {
            ...
            if base > u32::MAX as u64 {
                return Err(snapshot_error(format!(
                    "snapshot row offset overflow at entry {base}"
                )));
            }
        ...
        if base as usize != entries {
            return Err(snapshot_error(format!(
                "snapshot row window mismatch: degrees cover {base} entries, payload holds {entries}"
            )));
        }
        Ok(Self {
            map,
