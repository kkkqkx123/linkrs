...
fn test_nbr_at_offset_views_primary_slot() {
    ...
    csr.insert_edge(0u32, VertexId::edge_endpoint_key(7, 0), EdgeId(100), 1)
        .unwrap();
...
fn test_physical_reads_ignore_timestamps() {
    ...
    assert!(csr
        .get_edge_physical(0u32, VertexId::edge_endpoint_key(1, 0))
        .is_none());
...
fn test_physical_lookup_returns_rebuilt_live_edge() {
    ...
    csr.insert_edge(0u32, VertexId::edge_endpoint_key(1, 0), EdgeId(100), 10)
        .unwrap();
    assert!(csr.delete_edge(0u32, EdgeId(100), 20).unwrap());
    csr.insert_edge(0u32, VertexId::edge_endpoint_key(1, 0), EdgeId(101), 30)
        .unwrap();
    let found = csr
        .get_edge_physical(0u32, VertexId::edge_endpoint_key(1, 0))
...
fn consolidated_row_reads_single_block() {
    ...
    let hit = csr
        .get_edge(0u32, VertexId::edge_endpoint_key(17, 0), 1)
