...
use crate::storage::edge::{Csr, CsrBase, Nbr};
...
impl ColdDelta {
    ...
    ) -> HashMap<(u32, VertexId), crate::storage::edge::ImmutableNbr> {
...
impl ColdSnapshot {
    ...
    pub fn apply_delta(&self, delta: &ColdDelta) -> StorageResult<Self> {
        ...
        for edge in &delta.added {
            let prop_offset = if edge.properties.is_empty() {
                0
            } else {
                properties.insert(&edge.properties, edge.timestamp)?
            };
            out_entries.push((
                edge.src_internal,
                Nbr::new(edge.neighbor, edge.edge_id, prop_offset, edge.timestamp),
            ));
        }
        ...
        for (src, nbr) in &out_entries {
            ...
            if let Some(dst) = dst_vid.as_int64() {
                ...
                in_rows.push((
                    dst_row,
                    Nbr::new(dst_key, nbr.edge_id, nbr.prop_offset, nbr.create_ts),
                ));
            }
        }
        ...
        let property_index = if let Some(index) = self.property_index() {
            let exported = crate::storage::edge::edge_table::snapshot::ExportedEdgeSnapshot {
