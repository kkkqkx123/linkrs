...
mod tests {
    ...
    fn checked_internal_vertex_id_rejects_non_integer_ids() {
        assert!(checked_internal_vertex_id(
            &VertexId::try_from_string("vertex-a").expect("test vertex id")
        )
        .is_err());
    }
    ...
    fn checked_internal_vertex_id_rejects_values_outside_u32() {
        assert!(checked_internal_vertex_id(
            &VertexId::try_from_int64(i64::from(u32::MAX) + 1).expect("test vertex id")
        )
        .is_err());
    }
}
