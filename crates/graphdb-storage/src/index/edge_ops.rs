...
use crate::index::traits::EdgeIndexOps;
        ...
        let delta = {
            ...
            if !new_values.is_empty() {
                ...
                if let Some(pending) = pending_guard.get(&identity) {
                    let mut scan = crate::index::index_data_manager::PendingExistingScan {
                    ...
                    crate::index::index_data_manager::merge_pending_existing_values(
                        pending,
                        &reverse_prefix.0,
                        &reverse_end.0,
                        write_ts,
