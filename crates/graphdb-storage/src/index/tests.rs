...
fn included_columns_survive_rebuild_from_snapshot() {
    ...
    let checkpoint_dir = directory
        ...
        .join("shard-0");
    let fwd_idx = crate::index::chunk::chunked_index::ChunkedIndex::from_btree(
        vec![],
        &forward,
        64 * 1024 * 1024,
    );
    let rev_idx = crate::index::chunk::chunked_index::ChunkedIndex::from_btree(
        vec![],
        &reverse,
        64 * 1024 * 1024,
    );
    crate::index::chunk::serialize::write_chunked_index_checkpoint(
