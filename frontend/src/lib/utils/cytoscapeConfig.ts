...
export function convertToCytoscapeElements(data: GraphData): cytoscape.ElementDefinition[] {
  const nodes = data.nodes.map((node) => ({
    data: { id: node.id, label: node.tag, ...node.properties, _tag: node.tag },
  }));
  const edges = data.edges.map((edge) => ({
    data: { id: edge.id, source: edge.source, target: edge.target, label: edge.type, ...edge.properties, _type: edge.type, _rank: edge.rank },
  }));
  return [...nodes, ...edges];
}
...
export function generateCytoscapeStyle(config: GraphStyleConfig): cytoscape.StylesheetCSS[] {
  const baseNodeStyle: cytoscape.Css.Node = {
    ...
    'font-size': '12px', 'text-valign': 'center', 'text-halign': 'center', 'color': '#333',
    'text-outline-color': '#fff', 'text-outline-width': 1,
  };
  const baseEdgeStyle: cytoscape.Css.Edge = {
    'width': 2, 'line-color': '#ccc', 'curve-style': 'bezier',
  ...
  return [
    ...
    { selector: ':selected', css: { 'border-width': 3, 'border-color': '#1890ff', 'border-opacity': 1 } as cytoscape.Css.Node },
  ...
  const processValue = (value: unknown) => {
    ...
    if (obj.vid !== undefined) {
      ...
      if (!nodeIds.has(vid)) {
        ...
        nodes.push({ id: vid, tag: firstTag, properties: tags?.[firstTag] || {} });
      }
    }
    if (obj.srcID !== undefined && obj.dstID !== undefined && obj.edgeName !== undefined) {
      const srcId = String(obj.srcID), dstId = String(obj.dstID), edgeType = String(obj.edgeName);
      const rank = typeof obj.rank === 'number' ? obj.rank : 0;
      edges.push({ id: `${edgeType}_${srcId}_${dstId}_${rank}`, type: edgeType, source: srcId, target: dstId, rank, properties: (obj.properties as Record<string, unknown>) || {} });
      if (!nodeIds.has(srcId)) { nodeIds.add(srcId); nodes.push({ id: srcId, tag: 'unknown', properties: {} }); }
      if (!nodeIds.has(dstId)) { nodeIds.add(dstId); nodes.push({ id: dstId, tag: 'unknown', properties: {} }); }
    }
  };
  data.forEach((row) => {
    if (Array.isArray(row)) row.forEach(processValue); else processValue(row);
  });
  return { nodes, edges };
}