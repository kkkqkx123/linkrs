...
async fn create_test_web_state() -> (
    graphdb_server::web::WebState<graphdb::storage::GraphStorage>,
    i64,
) {
    ...
    use graphdb_server::http::AppState;
    use graphdb_server::http::HttpServer;
    use graphdb_server::web::{storage::SqliteStorage, WebState};
...
async fn test_query_history_lifecycle() {
    ...
    let app = graphdb_server::web::create_router(web_state);
